...
public class PackSlicer extends Slicer {
    ...
    private static final Path assetsFolder = OraxenPlugin.get().getResourcePack().getPackFolder().toPath().resolve("assets/minecraft");
    ...
    public static void slicePackFiles() {
        ...
        try {
            ...
            new PackSlicer(assetsFolder).process(INPUTS);
    ...
    private static InputFile clip(String inputName, String outputName, Box box) {
        String inputPath = nameToPath("minecraft",
                inputName);
        String outputPath = nameToPath("minecraft", outputName);
        Box imageBox = new Box(0, 0, box.totalW(), box.totalH(), box.totalW(), box.totalH());
        return (new InputFile(inputPath)).outputs(new OutputFile[]{(new OutputFile(outputPath, imageBox)).apply((image) -> {
            ...
            clippedImage.getGraphics().drawImage(subImage, x, y, (ImageObserver) null);
            return clippedImage;
        })});
    }
    ...
    private static UnaryOperator<BufferedImage> flipFrameAxis() {
        return (image) -> {
            ...
            for (int frame = 0; frame < 2; ++frame) {
                graphics.drawImage(image.getSubimage(frame * frameWidth, 0, frameWidth, frameHeight), 0, frame * frameHeight, (ImageObserver) null);
            }
